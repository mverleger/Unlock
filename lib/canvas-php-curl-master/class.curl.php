<?php
  ...
  class CanvasCurl {
    ...
    private function initCurl() {
      ...
      $this->setOpt(CURLOPT_HTTPHEADER, array('Content-Type: application/json', "Authorization: Bearer ".$this->token));

	  require_once('config.php');
	  $this->setOpt(CURLOPT_SSL_VERIFYHOST, HOST_SSL_LOCATION);
	  $this->setOpt(CURLOPT_SSL_VERIFYPEER, PEER_SSL_LOCATION);
	  $this->setOpt(CURLOPT_CERTINFO, 1);
    }
...
//      echo "setURL: ".$this->baseURL . $this->urlPath . ((strpos($url, '?') !== false) ? '&' : '?') . 'per_page=100'."<br>";
